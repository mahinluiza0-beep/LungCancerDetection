...
def insert():
    ...
    if (re.search(regex, email)):
        ...
        a = False
    # validation
    if not fname.isalpha() or (fname == ""):
        ms.showerror("showerror", "Error") 
        ms.showinfo("Message", "please enter valid name")
    elif addr == "" or addr.isdigit() or not any(char.isalpha() for char in addr):
        ms.showerror("showerror", "Error")
        ...
        with conn:
            ...
            conn.commit()
            db.close()
            answer = ms.askokcancel("Confirmation", "Are you sure you want to continue?")
            if answer:
                ms.showinfo('Success!', 'Account Created Successfully!')
                window.destroy()
            else:
                ms.showinfo('Cancelled', 'Operation cancelled.')
            # window.destroy()
            window.destroy()
